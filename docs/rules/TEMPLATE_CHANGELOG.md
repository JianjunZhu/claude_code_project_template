...
| 日期 | 变更 | 原因 | 影响 |
|---|---|---|---|
| 2026-06-26 | **新增"Baseline 先行"推荐实践（`RESEARCH_RULES.md` §17）**：确定研究主题后，第一件事是确立一个可信 baseline（复用开源实现 / 已发表方法官方实现，或自行获取数据后训练），以 baseline 为标尺，再针对其**具体不足**或一处**方法学创新**提出研究方案；增益一律相对该 baseline、同等调优下比较。**强烈推荐、非强制**，特殊情形（纯理论 / 无可比基线）可在 `TASK_BRIEF.md` 说明后跳过。交叉引用第 0/7/8/16.1 节与 `RESEARCH_LOOP.md` 第 1 节；并在计划环（§1）挂一条"推荐起手式 · Baseline 先行"指针。 | 用户要求把"baseline 先行、以其为标尺提方案"固化为模板规则，避免无参照系地空谈创新。 | 非破坏性：在 `RESEARCH_RULES.md` 末尾新增 §17（"关联文档"前）、`RESEARCH_LOOP.md` §1 加一条指针；既有章节编号与交叉引用不变。两文件均属 `docs/rules/`，经 `update_from_template.sh` 同步到派生项目。 |
| 2026-06-26 | **里程碑 v0.4.2-template**：把 §16「科研攻关执行教训」（6 条真实会话人工校正）固化为标签；README 与 QUICKSTART 的派生 / 更新**示例**及 `update_from_template.sh` 的 tag 建议 / 迁移提示统一改用 `v0.4.2-template`。布局阈值类引用（"早于 v0.4.0-template"、迁移安全闸、`.template-sync` 标注）仍保留 `v0.4.0-template`，因新布局确自 v0.4.0 引入。 | 让 `bootstrap -r v0.4.2-template` / `update -r v0.4.2-template` 直接带上新增的攻关执行教训章（旧标签停在更早状态、拿不到 §16）。 | 非破坏性：仅版本引用与标签。 |
| 2026-06-26 | **新增科研攻关执行教训（`RESEARCH_RULES.md` §16）**：把真实攻关会话的人工校正凝练为 6 条执行纪律——① 负结论与正信号**同等严格**验证（方法学 artifact 两头骗人：先查训练-评测一致性、警惕伪重复虚高显著性、加"学到的量 vs 平凡基线"控制、好结果同样查方法学）；② 异常先跑"关掉创新的纯 baseline"对照再归因，base 自身崩则问题在基础配置不在创新；③ 盲调超参前先调研 prior art 论文＋官方代码，网格盲搜是末选；④ 降级 / 撤创新 / 换路线等重大方向变更不由 agent 单方拍板，须摊证据＋不确定性给 PI，dev 小尺度负结果不足以撤创新；⑤ 算力吃满但不超占（填满已占卡的利用率/显存、停掉已证否实验）；⑥ 被定向时穷尽设计空间后负结论才可信、不轻易收尾。各条交叉引用 `CLAUDE.md` §8/§14.1/§15/§16 与本文件 §3/§4/§5/§7/§15，不重复既有纪律。 | 用户提供真实科研攻关中被反复证伪的判断，要求固化为模板规则，避免同类方法学错误再次误导降级 / 撤 claim / 调参 / 定方向。 | 非破坏性：仅在 `RESEARCH_RULES.md` 末尾新增 §16（"关联文档"前）；既有章节编号与交叉引用不变。该文件属 `docs/rules/`，经 `update_from_template.sh` 同步到派生项目。 |
| 2026-06-24 | **里程碑 v0.4.1-template**：把 post-v0.4.0 的两项改进（RESEARCH_LOOP 三环重设计 + 全模板优化：自检 linter / QUICKSTART / .editorconfig / 脚本微修 / 保守 DRY）固化为标签；README 与 QUICKSTART 的派生 / 更新**示例**统一改用 `v0.4.1-template`。布局阈值类引用（"早于 v0.4.0-template"、迁移安全闸）仍保留 `v0.4.0-template`，因新布局确自 v0.4.0 引入。 | 让 `bootstrap -r v0.4.1-template` / `update -r v0.4.1-template` 直接带上新循环协议与新文件（旧标签停在更早状态、拿不到这些）。 | 非破坏性：仅版本引用与标签。 |
| 2026-06-24 | **全模板优化（5 路审计后落地三档）**（post-v0.4.0）：① 新增 `scripts/check_template.sh` 自检 linter（机械不变量：无扁平引用、链接可解析、无 `math` 围栏、`bash -n`、ALLOW 齐备、rules/records 结构、`.gitkeep`），可挂 pre-commit/CI；② `bootstrap` 加 `trap` 清理临时文件、空目录判断改 `find`，`update_from_template` 加可选 `--timeout`、ALLOW 纳入 `QUICKSTART.md`/`.editorconfig`/`check_template.sh`；③ 新增 `.editorconfig`、`QUICKSTART.md`（首次清单 + **明确标注的合成示例**）；④ CLAUDE §12 点明非论文任务不必进 RESEARCH_LOOP、§18 注明 task_types 按需选用；⑤ 保守 DRY：证据等级以 `RESEARCH_RULES` 第 0 节为单一权威源，`EVIDENCE`/`README`/`MEMORY` 改为速查＋指针（不动 CLAUDE/RESEARCH_RULES 的自包含）。经 5 路并行审计；自检 linter 实测全绿、含负向测试。 | 用户要求"再检查整个模板还能优化什么"；审计后只落地真正有价值项（自检自动化、上手材料、降漂移），并明确拒绝过度 DRY（不外迁 CLAUDE.md 首读自包含内容）。 | 非破坏性：新增 2 文件 + 1 脚本、若干文档澄清与指针；既有规则与编号不变。新文件经 ALLOW 同步到旧项目。 |
