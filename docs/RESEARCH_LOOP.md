...
> **减少自评偏差**：检查环节优先借 `academic-paper-reviewer` 做**独立同行评审**，而非由执行者自评"看起来不错"。评审时对候选**随机标号、盲化**后再判，去除"偏向自家写法"的偏差；评审结论是参考意见（CLAUDE.md 第 17 节），但其指出的 critical 项必须落实到第 6 节状态表。
...
**实验条件覆盖清单**（设计阶段显式枚举，登记进 `docs/EXPERIMENT_LOG.md`）：基线 / 消融 / seed 方差 / 数据规模 / 分布漂移（OOD）/ 泄漏控制 / 失败-恢复 / 算力预算等须评估的条件；连续 ≥ 3 轮无新增条件即**条件饱和**。未覆盖条件数并入第 4 节"剩余工作量"。
...
5. **keep / discard**（autoresearch 精华）：指标改善且无回归（"无回归"以统计检验为机械判据，见 CLAUDE.md 第 8 节）→ 保留；变差 / 崩溃 / 无效 → 回退（如 `git revert`），失败零成本、历史单调趋好；**负结果归档不删除**（第 7 节、CLAUDE.md 第 6 节）。
...
>
> **假设驱动调试**：实验"看着不对"（疑似泄漏 / seed / 预处理 bug / 指标算错）时，把疑因当**可证伪假设**——每轮一条，用具名技术（二分 / 差分-ablation / 最小复现 / `git bisect` / 溯因）证实 / 证伪 / 不确定，证据记 file:line（`EXPERIMENT_LOG.md` 加 `调试假设｜技术｜判定｜证据` 列），勿随手乱改污染实验有效性。
...
- **剩余工作量** = 未达标 claim 数 + **失败 / 未通过的实验或测试数** + **未覆盖实验条件数** + 评审未决 critical 项数 + 未解决引用错误数。该值应**单调下降**才算收敛。
