...
任务执行阶段，在**不牺牲正确性、可复现与证据等级**的前提下追求最快的有效计算；长任务与长时间自主执行要可监控、**防卡死**、可快速止损；放量前先小批量跑通。
...
对训练 / 大批量推理 / 数据处理等**长时间运行**的任务，以及**长时间自主执行**（多轮循环、Workflow 编排、批处理）的过程，设一个**监察者**角色：**定时巡检、发现停滞即快速处置**——绝不让程序或 agent 长期卡死在某处不前进。

- **巡检节奏**：设固定巡检间隔（**默认约 30 分钟，按任务节奏调整**——快任务更短、大训练可更长）；每次巡检判断"自上次以来是否有实质进展"。
- **三类卡死来源都要查**：
  - **代码 / 任务逻辑**：进程存活但日志停滞 / 无新输出、loss·指标 `NaN` / 发散、死循环、GPU 利用率异常掉零、OOM、磁盘满、checkpoint 不按期产出。
  - **agent / 编排逻辑**：反复重复同一动作无进展、空转等待、循环不收敛、卡在某一步不动、超出预期轮次仍无产出。
