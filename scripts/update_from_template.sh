...
可选：
  -r, --ref <ref>         模板 ref（默认 main；建议用 tag，如 v0.2-template）
      --dry-run           只预览将更新哪些文件，不写入
  -f, --force             工作区不干净时仍继续（默认要求干净，便于 review）
  -h, --help

同步范围（模板拥有的文件）：
  CLAUDE.md, docs/RESEARCH_RULES.md, docs/RESEARCH_LOOP.md,
...
CLEANUP=""; STAGE=""
_cleanup() { [[ -n "$CLEANUP" ]] && rm -rf "$CLEANUP"; [[ -n "$STAGE" ]] && rm -rf "$STAGE"; }
trap _cleanup EXIT
...
echo
echo "✅ 已同步模板规则到 ${SRC_DESC}。项目自有内容（PROJECT / EVIDENCE / 实验 / 数据 / 结果）未改动。"
