...
可选：
  -r, --ref <ref>         模板 ref（默认 main；建议用 tag，如 v0.4.1-template）
...
if [[ ${#MIG_RM[@]} -gt 0 || ${#MIG_MV_SRC[@]} -gt 0 ]]; then
  exists "docs/rules/RESEARCH_RULES.md" \
    || die "模板 ref ${REF} 不含 docs/rules/（早于 v0.4.0-template）；为避免删除扁平规则却无新版本同步，已拒绝迁移。请改用 -r v0.4.1-template 或更新的 ref。"
fi
