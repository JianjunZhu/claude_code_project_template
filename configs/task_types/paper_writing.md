...
> 本文件仅在当前项目确属"论文写作"任务时启用；启用方式为在 `docs/records/PROJECT.md` 标注任务类型并按需引用本文件。所有具体取值用占位符表达，未知=TBD、未验证=PENDING、未确认=NOT VERIFIED、未完成=INCOMPLETE。
...
- 论文草稿与小节：`docs/records/PAPER_NOTES.md` 及稿件（路径 TBD）。
- claim → 证据映射表：每条 claim 链接到 `docs/records/EVIDENCE.md` 中具体条目与 artifact。
- 图表与其生成脚本/数据来源（图片相对路径引用）。
- 结果审计：`docs/records/RESULT_AUDIT.md` 中对论文数字的逐项核验。
- 参考文献与引用清单。
...
- 数字一致性：正文/图/表/附录中同一指标取值一致（取值 TBD/PENDING，源自 `docs/records/EVIDENCE.md`）。
...
1. `docs/records/EVIDENCE.md`（claim 的证据来源，最优先）。
2. `docs/records/RESULT_AUDIT.md`（数字核验）。
...
- 图表 + 生成脚本与数据来源记录（相对路径）。
- 数字一致性核验清单（引用 `docs/records/RESULT_AUDIT.md`）。
- 局限与负结果小节。
